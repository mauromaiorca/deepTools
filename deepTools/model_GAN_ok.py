...
class Discriminator3D(nn.Module):
    def __init__(self, in_channels=1, base_filters=64):
        ...
        self.conv1 = nn.Conv3d(in_channels, base_filters, kernel_size=4, stride=2, padding=1)
        self.lrelu1 = nn.LeakyReLU(0.2, inplace=True)
        
        self.conv2 = nn.Conv3d(base_filters, base_filters * 2, kernel_size=4, stride=2, padding=1)
        self.gn2 = nn.GroupNorm(num_groups=32, num_channels=base_filters * 2)
        self.lrelu2 = nn.LeakyReLU(0.2, inplace=True)
        
        # Change conv3's kernel size to 3 instead of 4 to handle smaller inputs.
        self.conv3 = nn.Conv3d(base_filters * 2, base_filters * 4, kernel_size=3, stride=2, padding=1)
        self.gn3 = nn.GroupNorm(num_groups=32, num_channels=base_filters * 4)
        self.lrelu3 = nn.LeakyReLU(0.2, inplace=True)
        
        # conv4 remains with kernel_size 3 as before.
        self.conv4 = nn.Conv3d(base_filters * 4, base_filters * 8, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x  # Returning raw logits for BCEWithLogitsLoss
