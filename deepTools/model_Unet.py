...
class LinformerAttention3D(nn.Module):
    def __init__(self, channels, reduce_ratio=4, r=64):
        ...
        # Fixed reduced sequence length r
        self.r = r
        ...
        # Eager instantiation of projection layers
        # Since we do not know N at init, we pick a fixed r and apply slicing later.
    ...
    def forward(self, x):
        ...
        q = self.query(x)  # [B, C, D, H, W]
        k = self.key(x)
        v = self.value(x)

        # Flatten
        q = q.view(B, C, N).permute(0, 2, 1)  # [B, N, C]
        k = k.view(B, C, N).permute(0, 2, 1)  # [B, N, C]
        v = v.view(B, C, N).permute(0, 2, 1)  # [B, N, C]

        # Project K, V
        k_t = k.permute(0, 2, 1)  # [B, C, N]
        v_t = v.permute(0, 2, 1)  # [B, C, N]

        k_proj = self.k_proj(k_t)  # [B, C, N]
        v_proj = self.v_proj(v_t)  # [B, C, N]

        # Slice to r (or less if N < r)
        max_r = min(self.r, k_proj.size(2))
        k_proj = k_proj[:, :, :max_r]  # [B, C, r']
        v_proj = v_proj[:, :, :max_r]  # [B, C, r']

        # Compute attention
        q_div = q / self.scale

        # attn_scores = (Q x K^T)
        # Q: [B,N,C], K_proj: [B,C,r']
        # Use einsum: 'b n c, b c r -> b n r'
        attn_scores = torch.einsum('b n c, b c r -> b n r', q_div, k_proj)

        attn_weights = F.softmax(attn_scores, dim=-1)  # [B, N, r']
        # attn_output = (attn_weights x V_proj)
        # 'b n r, b c r -> b n c'
...
class UNetWithAttention(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, reduce_ratio=4, r=64):
        ...
        self.inc = DoubleConv(in_channels, 64)  # Update in_channels to 1
        ...
        # Instantiate linear attention early
