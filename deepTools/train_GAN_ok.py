...
def main():
    ...
    prev_gen_state = generator.state_dict()
    prev_disc_state = discriminator.state_dict()
    prev_lambda_adv = lambda_adv
    prev_optimizer_G_state = optimizer_G.state_dict()
    prev_optimizer_D_state = optimizer_D.state_dict()

    for epoch in range(num_epochs):
        # Save current states for possible reversion later
        current_gen_state = generator.state_dict()
        current_disc_state = discriminator.state_dict()
        current_optimizer_G_state = optimizer_G.state_dict()
        current_optimizer_D_state = optimizer_D.state_dict()
        ...
        with torch.no_grad(), autocast():
            for inputs, targets, masks in val_loader:
                ...
                val_loss += Decimal(loss.item()) * Decimal(batch_size)
        ...
        # Check for NaN losses; if any loss is NaN, restore previous states and adjust parameters.
        if (not train_loss_G.is_finite()) or (not train_loss_D.is_finite()) or (not val_loss.is_finite()):
            print(f"NaN detected at epoch {epoch+1}. Reverting to previous state and adjusting hyperparameters.")
            # Restore the previous good state
            generator.load_state_dict(prev_gen_state)
            discriminator.load_state_dict(prev_disc_state)
            optimizer_G.load_state_dict(prev_optimizer_G_state)
            optimizer_D.load_state_dict(prev_optimizer_D_state)
            # Lower lambda_adv further (for example, reduce by half) to reduce the adversarial pressure.
            lambda_adv = max(1e-6, prev_lambda_adv * 0.5)
            # Optionally, reduce the learning rates
            for param_group in optimizer_G.param_groups:
                param_group['lr'] *= 0.5
            for param_group in optimizer_D.param_groups:
                param_group['lr'] *= 0.5
            print(f"New lambda_adv: {lambda_adv:.6f}. Learning rates reduced.")
            # Skip saving and updating best_val_loss; continue to next epoch.
            continue
        else:
            # Update previous state holders if this epoch is good.
            prev_gen_state = current_gen_state
            prev_disc_state = current_disc_state
            prev_optimizer_G_state = current_optimizer_G_state
            prev_optimizer_D_state = current_optimizer_D_state
            prev_lambda_adv = current_lambda_adv

        # -------------------------------
        # Dynamic adjustment of λ_adv:
        # Here we set a target discriminator loss (e.g. 0.1). If the actual discriminator loss is lower,
        # it indicates that the discriminator is very strong and we want to reduce the weight of the adversarial loss.
        target_d_loss = 0.1
        current_d_loss = float(train_loss_D) if train_loss_D.is_finite() else target_d_loss
        # Compute error (positive if the discriminator loss is below target)
        error = target_d_loss - current_d_loss
        # Update rule: multiply current lambda_adv by a factor that is 1 plus a small gain times the error.
        # For example, with gain = 0.1, if the discriminator loss is 0.05 (error = 0.05), then the factor is 1 + 0.005.
        gain = 0.1
    ...
    with torch.no_grad(), autocast():
        for inputs, targets, masks in test_loader:
            ...
            test_loss += Decimal(loss.item()) * Decimal(batch_size)
